...
const SidebarItem = ({ to, icon: Icon, label, active, onClick }: any) => (
  <Link
    ...
    className={`flex items-center space-x-3 px-4 py-3 rounded-xl transition-all duration-200 group ${
      active 
        ? 'bg-gray-800 dark:bg-primary-600 text-white shadow-lg shadow-gray-300 dark:shadow-primary-900/30' 
        : 'text-surface-600 dark:text-surface-400 hover:bg-surface-100 dark:hover:bg-surface-800 hover:text-primary-600'
    }`}
  >
...
const Layout: React.FC = () => {
  ...
  return (
    <div className="flex h-screen bg-white dark:bg-surface-900 transition-colors duration-300">
      {/* Mobile Sidebar Toggle */}
      <div className="lg:hidden fixed top-4 left-4 z-50">
        <button 
          onClick={() => setSidebarOpen(!isSidebarOpen)}
          className="p-2 bg-white dark:bg-surface-800 rounded-lg shadow-md hover:bg-surface-50 dark:hover:bg-surface-700 transition-colors border border-surface-200 dark:border-surface-700"
        >
      ...
      {/* Sidebar */}
      <aside className={`
        fixed inset-y-0 left-0 z-40 w-72 bg-white/90 dark:bg-surface-800/90 backdrop-blur-xl border-r border-surface-200 dark:border-surface-700 transition-transform duration-300 transform lg:relative lg:translate-x-0
        ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}
      `}>
        <div className="flex flex-col h-full p-6">
          <div className="flex items-center justify-between mb-10 px-4">
            <div className="flex items-center space-x-3">
              <div className="w-10 h-10 bg-gray-800 dark:bg-primary-600 rounded-xl flex items-center justify-center text-primary-500 dark:text-white font-bold text-xl shadow-lg shadow-gray-300 dark:shadow-primary-900/30">
                S
              </div>
          ...
          <div className="mt-auto pt-6 border-t border-surface-100 dark:border-surface-700">
            <div className="flex items-center space-x-3 px-4 py-3 mb-4 rounded-xl bg-surface-50 dark:bg-surface-900/50">
